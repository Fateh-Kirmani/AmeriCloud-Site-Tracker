...
const mockTeamMembers = [{ id: 'tm-1', name: 'Alice', email: 'alice@example.com', created_at: '2026-07-24T00:00:00Z' }]

beforeEach(() => { global.fetch = jest.fn() })
afterEach(() => jest.clearAllMocks())

it('shows loading then empty state when no team members', async () => {
  ;(fetch as jest.Mock).mockResolvedValueOnce({ ok: true, json: async () => ({ team_members: [] }) })
...
it('renders fetched team members', async () => {
  ;(fetch as jest.Mock).mockResolvedValueOnce({ ok: true, json: async () => ({ team_members: mockTeamMembers }) })
  render(<TeamTab projectId={PROJECT_ID} />)
...
it('calls PUT and refreshes on successful save', async () => {
  ;(fetch as jest.Mock)
    .mockResolvedValueOnce({ ok: true, json: async () => ({ team_members: [] }) })
    .mockResolvedValueOnce({ ok: true, json: async () => ({ team_members: [{ id: 'tm-new', name: 'Bob', email: null }] }) })
  render(<TeamTab projectId={PROJECT_ID} />)
  await waitFor(() => screen.getByText('No team members added yet.'))
