...
it('renders fetched milestones as input rows', async () => {
  ;(fetch as jest.Mock).mockResolvedValueOnce({
    ok: true,
    json: async () => [{ id: 'ms-1', details: 'Design Phase', owner: 'Alice', projected_date: '2026-08-01', actualized_date: '', notes: 'First' }],
  })
  render(<MilestonesTab projectId={PROJECT_ID} />)
...
it('removes a row when delete button is clicked', async () => {
  ;(fetch as jest.Mock).mockResolvedValueOnce({
    ok: true,
    json: async () => [{ id: 'ms-1', details: 'Phase 1', owner: '', projected_date: '', actualized_date: '', notes: '' }],
  })
  render(<MilestonesTab projectId={PROJECT_ID} />)
...
it('calls PUT and refreshes on successful save', async () => {
  ;(fetch as jest.Mock)
    ...
    .mockResolvedValueOnce({
      ok: true,
      json: async () => ({ milestones: [{ id: 'ms-new', details: 'New', owner: '', projected_date: '', actualized_date: '', notes: '' }] }),
    })
  render(<MilestonesTab projectId={PROJECT_ID} />)
  await waitFor(() => screen.getByText('No milestones added yet.'))
  fireEvent.click(screen.getByText('+ Add Milestone'))
