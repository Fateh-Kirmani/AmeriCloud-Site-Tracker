      ...
      <form onSubmit={handleSubmit(onSubmit)} aria-label="Edit project form" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="border-t border-[#1E3A5F] pt-4 space-y-4">
                <Field label="Name">
                  <input {...register('pm_name')} className={inputClass(false)} placeholder="PM full name" />
                </Field>
                <Field label="Email" error={errors.pm_email?.message}>
                  <input {...register('pm_email')} type="email" className={inputClass(!!errors.pm_email)} placeholder="pm@client.com" />
                </Field>
                <Field label="Phone">
                  <input {...register('pm_phone')} type="tel" className={inputClass(false)} placeholder="(555) 000-0000" />
                </Field>
              </div>
            </div>
            <div>
              ...
              <div className="border-t border-[#1E3A5F] pt-4 space-y-4">
                <Field label="Name">
                  <input {...register('rf_engineer_name')} className={inputClass(false)} placeholder="RF Engineer full name" />
                </Field>
                <Field label="Email" error={errors.rf_engineer_email?.message}>
                  <input {...register('rf_engineer_email')} type="email" className={inputClass(!!errors.rf_engineer_email)} placeholder="rf@client.com" />
                </Field>
                <Field label="Phone">
                  <input {...register('rf_engineer_phone')} type="tel" className={inputClass(false)} placeholder="(555) 000-0000" />
                </Field>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={() => router.push('/')}
            className="flex-1 border border-[#1E3A5F] text-[#94A3B8] hover:text-white hover:border-white font-semibold py-3.5 rounded-lg transition-colors text-sm uppercase tracking-widest"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 bg-[#C8102E] hover:bg-[#A50E25] disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-lg transition-colors text-sm uppercase tracking-widest shadow-lg"
          >
