...
export default function CrewTab({ projectId }: { projectId: string }) {
  const [rows, setRows] = useState<CrewMemberRow[]>([])
  ...
  const pendingDeleteRef = useRef<{ row: CrewMemberRow; index: number } | null>(null)
  ...
  async function save() {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/crew`, {
        ...
        body: JSON.stringify({ crew_members: rows, deleted_ids: [...deletedIds, ...extraIds] }),
      })
  ...
  return (
    <div className="space-y-3">
      {rows.length === 0 && <p className="text-[#94A3B8] text-sm">No crew members added yet.</p>}
      {rows.length > 0 && (
      ...
      {rows.map((row, i) => (
        <div key={i} className="flex gap-2 items-center">
          <input value={row.name} onChange={(e) => updateRow(i, 'name', e.target.value)} className={`${inputClass} flex-1`} placeholder="Full name" />
          <input value={row.email} onChange={(e) => updateRow(i, 'email', e.target.value)} className={`${inputClass} flex-1`} placeholder="email@company.com" type="email" />
          <input value={row.task} onChange={(e) => updateRow(i, 'task', e.target.value)} className={`${inputClass} flex-1`} placeholder="Task description" />
          ...
          <button type="button" onClick={() => deleteRow(i)} aria-label="Delete crew member" className="text-[#94A3B8] hover:text-[#C8102E] transition-colors shrink-0">
            <TrashIcon />
          </button>
        </div>
      ))}
      {showUndo && (
        <div className="flex items-center justify-between bg-[#1E3A5F] rounded-lg px-4 py-2 text-sm">
          <span className="text-[#94A3B8]">Crew member deleted.</span>
      ...
      <div className="flex items-center justify-between pt-2">
        <button type="button" onClick={addRow} className="text-[#94A3B8] hover:text-white text-sm font-medium transition-colors">+ Add Crew Member</button>
