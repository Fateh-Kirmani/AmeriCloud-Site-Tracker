...
import { TeamMemberRow } from '@/types/milestone'
...
export default function ManagerialTeamTab({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/projects/${projectId}/team`)
      .then((r) => r.json())
      .then(({ team_members }: { team_members: Array<{ id: string; name: string | null; email: string | null }> }) => {
        setRows(team_members.map((m) => ({ id: m.id, name: m.name ?? '', email: m.email ?? '' })))
      })
      .catch(() => setFetchError(true))
      .finally(() => setLoading(false))
  }, [projectId])

  function addRow() { setRows((r) => [...r, { name: '', email: '' }]) }
  ...
  async function save() {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/team`, {
        ...
        body: JSON.stringify({ team_members: rows, deleted_ids: [...deletedIds, ...extraIds] }),
      })
      if (!res.ok) throw new Error()
      const { team_members } = await res.json()
      setRows(team_members.map((m: { id: string; name: string | null; email: string | null }) => ({
